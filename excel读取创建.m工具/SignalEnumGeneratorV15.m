...
% �źź�ö�����ɹ��ߣ�֧��ö�١����ߡ��Զ�����ֵ���͡��ź�/���������������ļ���
% ֧�����ɼ��ؽű���һ���ָ����й��������󣨺�������Ϣ��
% �Ż����������ɲ��ٲ�����ʱ���������������ɾ�
% ������������UI�����ļ���������������У�顢���ؽű��ϲ�������ģʽ
% �����ԣ�����ʹ�� uifigure ϵ�ִ��ؼ���ȱ�����񲼾�ʱ���˵��ִ��ؼ�+���ز���
...
% ==================== �ص����� ====================
    function selectEnumTargetFolder()
        ...
        if folder ~= 0
            ...
            set(fig, 'UserData', appData);
            setStatus(sprintf('ö���ļ����·��: %s', folder), [0 0 0]);
        end
    end
    ...
    function addFiles(type)
        ...
        if isempty(validFiles)
            setStatus('δѡ����Ч���ļ�����ʱ�ļ��ѱ����ˣ�', [0.8 0.4 0]);
            return;
        end
    ...
    function addFolder(type)
        ...
        switch type
            case 'enum'
                ...
                appData.enumFiles = allFiles;
                updateListBox(appData.enumListBox, allFiles);
                setStatus(sprintf('���ļ����������� %d ��ö���ļ����� %d ��', length(excelFiles), length(allFiles)), [0 0 0]);
            case 'interface'
                excelFiles = findAllExcelFiles(folder);
                ...
                appData.interfaceFiles = allFiles;
                updateListBox(appData.interfaceListBox, allFiles);
                setStatus(sprintf('���ļ����������� %d �� Interface �ļ����� %d ��', length(excelFiles), length(allFiles)), [0 0 0]);
            case 'var'
                excelFiles = findAllExcelFiles(folder);
                ...
                appData.varFiles = allFiles;
                updateListBox(appData.varListBox, allFiles);
                setStatus(sprintf('���ļ����������� %d ���������������ļ����� %d ��', length(excelFiles), length(allFiles)), [0 0 0]);
            case 'script'
                ...
                appData.otherMFiles = allFiles;
                updateListBox(appData.otherMListBox, allFiles);
                setStatus(sprintf('���ļ����������� %d ������ .m �ļ����� %d ��', length(scriptFiles), length(allFiles)), [0 0 0]);
        end
        set(fig, 'UserData', appData);
    end
    ...
    function deleteSelected(type)
        ...
        switch type
            case 'enum'
                ...
                updateListBox(appData.enumListBox, appData.enumFiles);
                setStatus(sprintf('��ɾ�� %d ��ö���ļ���ʣ�� %d ��', length(idx), length(appData.enumFiles)), [0 0 0]);
            case 'interface'
                ...
                updateListBox(appData.interfaceListBox, appData.interfaceFiles);
                setStatus(sprintf('��ɾ�� %d �� Interface �ļ���ʣ�� %d ��', length(idx), length(appData.interfaceFiles)), [0 0 0]);
            case 'var'
                ...
                updateListBox(appData.varListBox, appData.varFiles);
                setStatus(sprintf('��ɾ�� %d ���������������ļ���ʣ�� %d ��', length(idx), length(appData.varFiles)), [0 0 0]);
            case 'script'
                ...
                updateListBox(appData.otherMListBox, appData.otherMFiles);
                setStatus(sprintf('��ɾ�� %d ������ .m �ļ���ʣ�� %d ��', length(idx), length(appData.otherMFiles)), [0 0 0]);
        end
        set(fig, 'UserData', appData);
    end
    ...
    function clearAll(type)
        ...
        switch type
            case 'enum'
                ...
                setStatus('�����ö���ļ��б�', [0 0 0]);
            case 'interface'
                appData.interfaceFiles = {};
                updateListBox(appData.interfaceListBox, {});
                setStatus('����� Interface �ļ��б�', [0 0 0]);
            case 'var'
                appData.varFiles = {};
...
% ==================== ���������� ====================
    function generate()
        ...
        if ~isempty(appData.enumFiles) && isempty(appData.targetPath)
            showAlert('error', 'ѡ����ö���ļ�����ѡ��ö���ļ����·����', uiText.errorTitle);
            return;
        end
        ...
        headerLines = {
            '%% Simulink ���������ݼ��ؽű�';
            '% �˽ű��� SignalEnumGenerator �Զ�����';
            '% ���д˽ű��ɻָ����й���������ö��, AliasType, Bus, Parameter, Signal������������Ϣ';
            '';
            };
        ...
        try
            for enumIdx = 1:length(appData.enumFiles)
                ...
                try
                    ...
                    errorLog = appendCellMessage(errorLog, sprintf('ö���ļ�����ʧ��: %s - %s', enumFile, ME.message));
                end
            end
            ...
            for intfIdx = 1:length(appData.interfaceFiles)
                ...
                try
                    ...
                    errorLog = appendCellMessage(errorLog, sprintf('Interface�ļ�����ʧ��: %s - %s', intfFile, ME.message));
                end
            end
            ...
            for varIdx = 1:length(appData.varFiles)
                ...
                try
                    ...
                    errorLog = appendCellMessage(errorLog, sprintf('���������ļ�����ʧ��: %s - %s', varFile, ME.message));
                end
            end
            ...
            if ~isempty(errorLog)
                ...
                setStatus('������ɣ������ڴ���', [0.8 0.5 0]);
                showAlert('warning', sprintf('���ɹ����з��� %d ��������鿴�����', length(errorLog)), uiText.warningTitle);
            else
                fprintf('\n[OK] ������ɣ����д����ɹ���\n');
                setStatus('[OK] �������', [0 0.5 0]);
                showAlert('success', sprintf('���ɳɹ���\n���ؽű��ѱ�����:\n%s', scriptPath), uiText.doneTitle);
            end
        catch ME
            setStatus('[FAIL] ����ʧ��', [0.8 0 0]);
            showAlert('error', sprintf('����ʧ��:\n%s', ME.message), uiText.errorTitle);
            rethrow(ME);
        end
        ...
        function lines = buildMergedScriptLines(filePaths)
            ...
            for fileIdx = 1:length(filePaths)
                ...
                if ~exist(scriptFile, 'file')
                    warningLog = appendCellMessage(warningLog, sprintf('���ӽű������ڣ�������: %s', scriptFile));
                    continue;
                end
                try
                    scriptContent = fileread(scriptFile);
                    scriptLines = regexp(scriptContent, '\r\n|\n|\r', 'split');
                    lines{end+1} = sprintf('%% ===== �ϲ��ű�: %s =====', scriptFile);
                    lines = [lines; scriptLines(:)];
                    lines{end+1} = sprintf('%% ===== ����: %s =====', scriptFile);
                    lines{end+1} = '';
                catch ME
                    warningLog = appendCellMessage(warningLog, sprintf('��ȡ���ӽű�ʧ�� %s: %s', scriptFile, ME.message));
                end
            end
        end
            ...
            try
                ...
                if ~isempty(enumGroups)
                    ...
                    for enumGroupIdx = 1:length(enumGroups)
                        ...
                        if fid == -1
                            errors = appendCellMessage(errors, sprintf('�޷�����ö���ļ�: %s', className));
                            continue;
                        end
                        fprintf(fid, '%% %s ö���ඨ��\n', className);
                ...
                errors = appendCellMessage(errors, sprintf('����ö��sheetʧ��: %s', ME.message));
            end
            try
                ...
                errors = appendCellMessage(errors, sprintf('����numeric sheetʧ��: %s', ME.message));
            end
            try
                ...
                errors = appendCellMessage(errors, sprintf('����bus sheetʧ��: %s', ME.message));
            end
        end
        ...
        function [enumGroups, errors] = parseEnumSheet(excelFile)
            ...
            try
                ...
                if isempty(rawData)
                    errors = appendCellMessage(errors, '�޷���ȡ "enumeration" sheet');
                    return;
                end
                [headerRowIdx, headers] = findHeaderRow(rawData, {'ConvName'});
                if headerRowIdx == 0
                    errors = appendCellMessage(errors, 'δ�ҵ� ConvName ��');
                    return;
                end
                convNameCol = findColumnIndex(headers, {'ConvName'});
                descCol = findColumnIndex(headers, {'Description', '����'});
                if convNameCol == 0
                    errors = appendCellMessage(errors, 'δ�ҵ� ConvName ��');
                    return;
                end
                ...
                errors = appendCellMessage(errors, sprintf('����ö��sheet�쳣: %s', ME.message));
            end
        end
        ...
        function [varNames, errors, warnings] = processNumericSheet(excelFile)
            ...
            try
                ...
                if headerRowIdx == 0
                    warnings = appendCellMessage(warnings, sprintf('%s: numeric sheet δ�ҵ� ConvName/DataType �У�����', excelFile));
                    return;
                end
                convNameCol = findColumnIndex(headers, {'ConvName'});
                ...
                if convNameCol == 0 || dataTypeCol == 0
                    warnings = appendCellMessage(warnings, sprintf('%s: numeric sheet ȱ�ٱ����У�����', excelFile));
                    return;
                end
                ...
                for row = 1:size(dataRows, 1)
                    ...
                    try
                        ...
                        errors = appendCellMessage(errors, sprintf('����AliasTypeʧ�� %s: %s', typeName, ME.message));
                    end
                end
            catch ME
        ...
        function [varNames, errors, warnings] = processBusSheet(excelFile)
            ...
            try
                ...
                if headerRowIdx == 0
                    warnings = appendCellMessage(warnings, sprintf('%s: bus sheet δ�ҵ� BusName/ElementName/Typedef �У�����', excelFile));
                    return;
                end
                ...
                busDescCol = findColumnIndex(headers, {'BusDescription', '��������'});
                elemDescCol = findColumnIndex(headers, {'ElementDescription', 'Ԫ������', 'Description', '����'});
                initValCol = findColumnIndex(headers, {'InitialValue', '��ʼֵ'});
                if busNameCol == 0 || elemNameCol == 0 || typeDefCol == 0
                    warnings = appendCellMessage(warnings, sprintf('%s: bus sheet ȱ�ٱ����У�����', excelFile));
                    return;
                end
                dataRows = rawData(headerRowIdx+1:end, :);
                ...
                for busIdx = 1:length(busNames)
                    ...
                    busLines = {};
                    busLines{end+1} = sprintf('%% ����: %s', busName);
                    ...
                    for j = 1:length(elements)
                        ...
                        signalLines = {};
                        signalLines{end+1} = sprintf('%% ���߳�Ա�ź�: %s (�������� %s)', signalName, busName);
        ...
        function [varNames, errors, warnings] = processInterfaceFile(excelFile)
            ...
            for sheetIdx = 1:length(sheets)
                ...
                try
                    ...
                    errors = appendCellMessage(errors, sprintf('������ %s ����ʧ��: %s', sheetName, ME.message));
                end
            end
        end
        ...
        function [varNames, errors, warnings] = processCalNvvSheet(rawData, numData, srcFile)
            ...
            descCol = findColumnIndex(headers, {'Description', '����'});
            if nameCol == 0
                warnings = appendCellMessage(warnings, sprintf('%s: δ�ҵ�Name�У�����', srcFile));
                return;
            end
            ...
            for row = 1:size(dataRows, 1)
                ...
                if ~valid
                    errors = appendCellMessage(errors, sprintf('���� %s ����������Ч: %s', varName, errMsg));
                    continue;
                end
                width = 1;
                ...
                if startsWith(dataTypeRaw, 'Enum:', 'IgnoreCase', true)
                    ...
                    try
                        if ~exist(enumClassName, 'class')
                            errors = appendCellMessage(errors, sprintf('ö���� %s δ�ҵ������� %s ����ʧ��', enumClassName, varName));
                            continue;
                        end
                        ...
                        if isnumeric(defaultValue)
                            ...
                            if ~isempty(members)
                                ...
                                if ~isempty(idx)
                                    ...
                                    warnings = appendCellMessage(warnings, sprintf('��ֵ %d δƥ��ö�� %s��ʹ�õ�һ����Ա', defaultValue, enumClassName));
                                    enumValue = members(1);
                                end
                            else
                                errors = appendCellMessage(errors, sprintf('ö���� %s �޳�Ա', enumClassName));
                                continue;
                            end
                        elseif ischar(defaultValue)
                            ...
                            try
                                ...
                                if ~isempty(idx)
                                    ...
                                    errors = appendCellMessage(errors, sprintf('ö���� %s ��δ�ҵ���Ա %s', enumClassName, memberName));
                                    continue;
                                end
                            end
                        else
                            errors = appendCellMessage(errors, sprintf('���� %s Ĭ��ֵ���Ͳ�֧��', varName));
                            continue;
                        end
                        ...
                        errors = appendCellMessage(errors, sprintf('����ö�ٲ���ʧ�� %s: %s', varName, ME.message));
                    end
                else
                    try
                        param = Simulink.Parameter;
                        ...
                        errors = appendCellMessage(errors, sprintf('��������ʧ�� %s: %s', varName, ME.message));
                    end
                end
            end
        end
        ...
        function [varNames, errors, warnings] = processSignalSheet(rawData, srcFile)
            ...
            for row = 1:size(dataRows, 1)
                ...
                if ~valid
                    errors = appendCellMessage(errors, sprintf('�ź� %s ����������Ч: %s', sigName, errMsg));
                    continue;
                end
                initVal = '0';
                ...
                try
                    ...
                    errors = appendCellMessage(errors, sprintf('�����ź�ʧ�� %s: %s', sigName, ME.message));
                end
            end
        end
        ...
        function [varNames, errors, warnings] = processVariableDefinitionFile(excelFile)
            ...
            try
                ...
                if isempty(sheetNames)
                    warnings = appendCellMessage(warnings, sprintf('�ļ� %s ����Ч������������', excelFile));
                    return;
                end
                ...
                errors = appendCellMessage(errors, sprintf('�������������ļ�ʧ��: %s', ME.message));
            end
        end
        ...
        function [valid, errMsg] = validateDataType(dataType)
            ...
            if evalin('base', sprintf('exist(''%s'', ''var'')', dataType)) && evalin('base', sprintf('isa(%s, ''Simulink.AliasType'')', dataType))
                ...
                valid = false;
                errMsg = sprintf('�������� %s �����������͡�ö�١�Bus���Ѷ����AliasType', dataType);
            end
        end
    end
    ...
    function mergeLoadScript(scriptPath, scriptMap, scriptOrder, generatedVars, extraScriptLines)
        ...
        if isempty(newVarSet) && exist(scriptPath, 'file')
            fprintf('[OK] δ��⵽�µı����򸽼ӽű����������м��ؽű�����: %s\n', scriptPath);
            return;
        end

        if ~exist(scriptPath, 'file')
            fid = fopen(scriptPath, 'w');
            if fid == -1, error('�޷������ű��ļ�'); end
            fprintf(fid, '%s\n', generatedMarker);
